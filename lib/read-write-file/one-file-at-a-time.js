...
import {
  tryWriteFile,
  readOneFileAtATime,
  controlFlowForAwaitLoop,
} from "../../utils/index.js";

const SRC_RELATIVE_PATH =
  "F:/My Drive/insnare/client/australiance/medtech-cpy-profile";
...
const DEST_REL_PATH = "medtech-cpies";
const DEST_ABS_PATH = path.resolve(DEST_FOLDER_PATH, DEST_REL_PATH);
...
//         : new Date().getTime(); // new Date().toLocaleDateString()
//     try {
//       // synchronous processing of `json_arr`
//       const nested_normalized_recs = json_arr.map((rec) => {});
// // write to file
// const base_name = `from-${file_timestamp}-${DEST_REL_PATH}-${json_arr.length}-profiles`;
// await tryWriteFile(DEST_ABS_PATH, base_name, FILE_EXT, json_arr);
...
async function processWriteOneFileAtATime() {
  ...
  for await (const { dirent, json_arr } of reader) {
    try {
      ...
      await controlFlowForAwaitLoop(); /* To control the flow of the `for await...of` loop, I used the promisified version `fs.promises.writeFile()`.
      `fs.promises.writeFile()` returns a Promise, which is why `tryWriteFile()` was made async.
      The `for await...of` loop releases one file at a time.
      Thereby, inside the loop, the processing of `json_arr` is synchronous.
      Therefore, it's the `for await...of` loop that's responsible for putting backpressure on `fs.promises.writeFile()`.  
...
async function main() {
  ...
  try {
    ...
    // write to file
    const base_name = `from-${new Date().getTime()}-${DEST_REL_PATH}-${
      acc.length
    }-records`;
    await tryWriteFile(DEST_ABS_PATH, base_name, FILE_EXT, acc);
  } catch (error) {
    // preserve stack trace whilst adding context
    error.message = `Error in main process: ${error.message}`;
    throw error;
  } finally {
    console.timeEnd("Script Execution Time");
  }
